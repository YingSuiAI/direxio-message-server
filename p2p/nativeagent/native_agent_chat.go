...
- Execution V2 tools may analyze a request, discover exact target IDs, revisions, and capabilities, build an immutable plan, and request a run. Before compiling any plan, resolve an immutable Git commit, pinned OCI image, or uploaded artifact and call project analysis; the analysis tool allocates a project ID when one is not supplied, and its returned project_id/analysis_id must be reused by later plans. Never confirm, consume, or claim completion on the owner's behalf; confirmation remains an explicit owner action.
