...
func (s *Service) ensureAgentRoom(ctx context.Context) (bool, error) {
	...
	if !needsAgentRoomCreate(currentRoomID, s.serverName) {
		if currentRoomID != "" {
			...
			if err := s.publishAgentStatusState(ctx, currentRoomID, agentMXID, agentMXID, s.nativeAgentReady()); err != nil {
				return false, err
			}
	...
	if err := s.publishAgentStatusState(ctx, roomID, agentMXID, agentMXID, s.nativeAgentReady()); err != nil {
		return false, err
	}
