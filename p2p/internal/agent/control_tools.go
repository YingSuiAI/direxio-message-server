...
func ControlTools(control ControlInvoker) []nativeagent.Tool {
	...
	return []nativeagent.Tool{
		...
		tool("native_agent_deployments_get", "Get an owner deployment.", "agent.core.deployments.get", strictObjectSchema(map[string]any{"workload_id": stringSchema()}, "workload_id"), []string{"workload_id"}, false, false),
		tool("native_agent_deployments_events", "List owner deployment events.", "agent.core.deployments.events", strictObjectSchema(map[string]any{"workload_id": stringSchema(), "after_sequence": integerSchema(), "page_size": integerSchema()}, "workload_id"), []string{"workload_id", "after_sequence", "page_size"}, false, false),
