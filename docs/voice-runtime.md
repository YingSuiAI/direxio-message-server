...
speech profile credential bundle. Agent's direct catalog advertises the voice
capability only when these URLs, the webhook secret, and the profile store are
available.
...
are the normal ASR path. Agent maps each callback deterministically to its
durable turn in the selected conversation. `turn.done` completes a turn;
`session.done` is reserved for session termination.
