...
Native Agent is a first-class server feature behind owner `agent.*` actions. The existing local Runner remains available for compatibility, while Chat, StreamChat, runtime model-profile selection, and Team Plan orchestration can use the independent `dirextalk-agent` service. Clients call `agent.chat`、`agent.team.plans.get`、`agent.team.approval_device.bootstrap`、`agent.team.plans.approval.prepare`、`agent.team.plans.approve`、`agent.team.executions.get`、`agent.models.list`、`agent.runtime.inspect/install/which/run`、`agent.skills.*`、`agent.mcp.*`、`agent.context.compress`、`agent.config.propose_patch` and built-in Dirextalk tool actions directly through `/query` or `/command` with owner `access_token`; `agent_token` is limited to `agent.matrix_session.create` through the product body-action surface and `POST /mcp`.
...
Remote search-provider selection uses the same catalog-bound security model through owner-only, HTTP-only and no-store `agent.search.profile.get/update`. Public state contains only profile ID, provider, HTTPS endpoint and bounded `max_results`/`timeout_seconds`; API keys and secret references remain in Agent-mounted operator secrets and are not representable in ProductCore requests or responses. Search profile updates require an existing model runtime configuration, use the shared runtime revision fence, preserve model, memory, knowledge, MCP, recipe and project settings, and add the reserved `web_search` capability to the normalized tool allow-list. The independent Agent still re-resolves the profile and owns credential access plus provider execution. `agent.cloud.tasks.overview` is the owner-scoped exact task summary used by clients for persistent task status UI; it returns one aggregate snapshot and bounded recent task summaries rather than making clients count paginated task-list pages.
