...
- Remote Chat is disabled by default. Enabling it requires `P2P_AGENT_GRPC_ENABLED=true` plus `P2P_AGENT_GRPC_TARGET`, `P2P_AGENT_GRPC_CA_FILE`, `P2P_AGENT_GRPC_SERVER_NAME`, and an absolute `P2P_AGENT_GRPC_SERVICE_KEY_FILE`. The client uses TLS 1.3, a mounted pairwise `DTX-Service-Key`, and the stable non-Matrix owner identifier `dirextalk-project:<server_name>`. Inline `P2P_AGENT_GRPC_SERVICE_KEY` is forbidden. Incomplete configuration or client construction fails closed and never falls back to local Chat.
- The current remote slice supports ordinary prompt-based Chat only. It forwards a UUID idempotency key, stable conversation ID, and exact conversation revision. Legacy `conversation_context`, `model_profile_id`, and `model_profile` are accepted only for client compatibility and are never serialized to Agent gRPC; Cloud dialogue, Knowledge/Embedding, attachments, system prompts, and caller-selected tools fail closed until typed Agent contracts exist. A remote stream succeeds only after exactly one `done` event followed by EOF.
...
- Model API keys remain request-scoped only for the local fallback Runner. The Message Server must not persist or return them, and the remote gRPC adapter discards the complete legacy model profile before crossing the Agent boundary. The independent Agent reads its model credential from a protected mounted secret.
