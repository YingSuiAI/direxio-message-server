...
- The service must open the PostgreSQL-backed store successfully; migration or connection failure is fatal for the product runtime and must not silently fall back to in-memory state.
...
docker volume rm dirextalk-p2p_p2p_message_server_config
```
...
Plugin management actions remain owner-only compatibility actions, but they are deprecated and outside the current acceptance scope. Native Agent uses first-class owner `agent.*` actions and `client.native_agent_stream` realtime frames; `io.dirextalk.agent` is not returned or accepted by plugin catalog/list/lifecycle/config/invoke/health/log surfaces. The production `docker-compose.p2p.yml` keeps `P2P_PLUGIN_DOCKER_ENABLED=false`, does not mount `/var/run/docker.sock`, and does not start a Docker-plugin runner, Agent sidecar, or external Agent Core/gRPC service. The Message Server and its one-shot initializer use the same application image; PostgreSQL remains separate infrastructure. Native Agent model profiles are server-owned encrypted records with default conversation roles. Chat, stream, and compression omit model fields and resolve the owner default conversation profile; inline `model_profile`/key inputs remain legacy compatibility-only and are not persisted, returned, or injected into any container env. `agent.models.list` remains an explicit request-scoped lookup and does not persist profiles. Native Agent runtime config, including shared display/avatar settings, `mcp_blocked_room_ids`, skills, third-party MCP client metadata, runtime CLI metadata, and memory settings, is stored in the native portal Agent config JSON; any sanitized legacy model-profile fields in config are compatibility-only. Startup performs an idempotent sanitized import from old hidden `io.dirextalk.agent` plugin config when present; API key and API key ref fields are stripped during import and on native config save/load. Runtime CLI files, installed skill files, MCP working data, server-side memory, context compression artifacts, and Agent orchestration data live in the native Agent data dir configured by `P2P_NATIVE_AGENT_DATA_DIR`. Agent knowledge action names are reserved but first-version runtime reports unsupported and does not load vector index dependencies. The backend keeps the standard `POST /mcp` endpoint for agent-token MCP access. Ops owns status, backup, migration export, cleanup planning, backup progress polling, chunked backup download, and explicit confirmed restore from existing backup records. Direct SQL purge of Matrix event history is not part of the first-version Ops contract.
当前 x1 本机部署固定落在单节点 `docker-compose.p2p.yml` 的 `message-server` 容器上；三节点 `dendrite-a`/`b`/`c` 仅用于 multi-node regression，不承载 x1 的实际对外域名，也不安装或承载插件。
