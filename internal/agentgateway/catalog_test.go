...
func TestCloudWorkerExecutionCatalogPinsRecordKindInputSchemas(t *testing.T) {
	tests := []struct {
		...
		{"agent.execution.v2.plans.get", "plans_get", `{"additionalProperties":false,"properties":{"plan_id":{"type":"string"},"record_kind":{"enum":["cloud_worker"],"type":"string"},"revision":{"type":"integer"}},"required":["record_kind","plan_id"],"type":"object"}`},
		{"agent.execution.v2.plans.list", "plans_list", `{"additionalProperties":false,"properties":{"page_size":{"type":"integer"},"page_token":{"type":"string"},"record_kind":{"enum":["cloud_worker"],"type":"string"}},"required":["record_kind"],"type":"object"}`},
		{"agent.execution.v2.runs.get", "runs_get", `{"additionalProperties":false,"properties":{"record_kind":{"enum":["cloud_worker"],"type":"string"},"run_id":{"type":"string"}},"required":["record_kind","run_id"],"type":"object"}`},
