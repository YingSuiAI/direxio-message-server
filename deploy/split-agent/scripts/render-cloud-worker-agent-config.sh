...
for suffix in ARTIFACT_RETENTION QUOTE_TTL MAX_RUNTIME CONTROLLER_POLL_INTERVAL WORKER_HEARTBEAT_INTERVAL REAPER_INTERVAL COMPLETION_OUTBOX_INTERVAL; do
  match "DIREXTALK_CLOUD_WORKER_$suffix" '^[1-9][0-9]*(ms|s|m|h)$'
done
[ "$DIREXTALK_CLOUD_WORKER_ARTIFACT_RETENTION" = 720h ] || die "ARTIFACT_RETENTION must be 720h (30 days)"
match DIREXTALK_CLOUD_WORKER_MAXIMUM_CATALOG_AGE '^(0s|[1-9][0-9]*(ms|s|m|h))$'
