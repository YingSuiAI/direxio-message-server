...
case "$1 $2" in
  ...
  'inspect message-id') printf '[{"Config":{"Image":"docker.io/dirextalk/message-server:latest"},"Image":"sha256:message","State":{"Health":{"Status":"healthy"}}}]\n' ;;
  'inspect agent-id'|'inspect extension-id'|'inspect core-id') printf '[{"Config":{"Image":"docker.io/dirextalk/agent:latest"},"Image":"sha256:agent","State":{"Health":{"Status":"healthy"}}}]\n' ;;
...
sed -i 's#dirextalk/agent:latest#dirextalk/agent:v1.0.69#' "$tmp/.env"
if PATH="$tmp/bin:$PATH" "$script_dir/verify-production-images.sh" "$tmp/.env" >/dev/null 2>&1; then echo 'versioned Agent tag unexpectedly accepted' >&2; exit 1; fi
printf 'production latest image and running version checks verified\n'
